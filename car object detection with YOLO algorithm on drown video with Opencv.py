#Yakov Mishayev and Idan Nesimov
...
def draw_boxes(image, v_boxes, v_labels, v_scores):
    ...
    for i in range(len(v_boxes)):
        ...
        cv2.circle(image, ((x2 + x1)//2,(y2 + y1)//2), 4, (0, 0, 255), -1)


        # draw text and score in top left corner
        label = "%s (%.3f)" % (v_labels[i], v_scores[i])
        cv2.putText(image, label, (x1,y1-20), cv2.FONT_HERSHEY_SIMPLEX, 1,
                    (0, 0, 255), 2)
    # show the plot
...
while (cap.isOpened()):
    ...
    image, image_w, image_h = load_image_pixels(image),416,416
